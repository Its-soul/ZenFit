...
export default function MealAnalysisPage(){
 ...
 const decision=analysis?.recognition_decision;
 ...
 return <ProtectedFeaturePage title="Meal analysis" description="Upload a meal photo, review every estimate, and correct portions before saving.">
   <div className="grid gap-5 lg:grid-cols-[1fr_1.15fr]">
    ...
    <section className="panel rounded-2xl p-5"><h2 className="font-semibold">Review foods</h2><p className="mt-1 text-sm text-muted">Image portions are estimates. Change names, pieces, or grams before confirming.</p>
     {analysis?.recognition_message?<p className={`mt-3 rounded-xl p-3 text-sm ${decision==="SUPPORTED_FOOD"?"bg-emerald-400/10 text-zenSage":decision==="NON_FOOD"?"bg-amber-400/10 text-amber-200":"bg-white/5 text-muted"}`}>{analysis.recognition_message}</p>:null}
     {decision==="LOW_CONFIDENCE"&&analysis?.top_candidates?.length?<div className="mt-2 flex flex-wrap gap-2">{analysis.top_candidates.map(candidate=><button key={candidate.label} className="rounded-full bg-white/10 px-2 py-1 text-xs" onClick={()=>setFoods([{...blank,name:candidate.label}])}>{candidate.label.replaceAll("_"," ")}</button>)}</div>:null}
