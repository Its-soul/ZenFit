...
Historical source-controlled summaries are retained in `MODEL_CARDS.md`; binary weights and full generated evidence live under ignored `data/models/` or external artifact storage.
...
|---|---|---|---|---|---|---|
| BGE-M3 | 1024d memory embeddings | `BAAI/bge-m3` | MIT | `AI_MODEL_CACHE_DIR` | Core memory | `python -m scripts.setup_models`; model card permits local use. |
...
The Indian-food classifier supports 14 classes with technically ready closed-set quality. Candidate 1.1.0 remains inactive: unknown-food rejection, non-food rejection, and open-set evidence size are blocked. Its 0.57/0.28/0.10 thresholds are experimental only. No real model was loaded during the open-set build-only stage.
