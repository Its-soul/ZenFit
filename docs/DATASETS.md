...
```powershell
docker compose exec backend python -m training.download_kaggle_datasets
docker compose exec backend python -m training.audit_kaggle_datasets
docker compose exec backend python -m training.prepare_kaggle_indian_food
```
# Open-set evaluation data

No new evaluation dataset was downloaded in the build-only stage. Future supported/unknown/non-food evidence must use developer-owned or legally usable images, retain per-image provenance and license metadata, remain outside Git when private/large, and satisfy the configurable project evidence minimums documented in `OPEN_SET_EVALUATION.md`.
