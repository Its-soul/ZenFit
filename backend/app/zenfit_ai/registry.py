...
class ModelRegistry:
    ...
    def get_food_classifier(self):
        from app.zenfit_ai.meal_scan.classifier import load_classifier
    ...
    def status(self) -> dict[str, bool]:
        ...
        return {
            "bge_embeddings": self.error("bge_embeddings") is None and (self.loaded("bge_embeddings") or (importlib.util.find_spec("sentence_transformers") is not None and cached(self.settings.embedding_model))),
            "bge_reranker": self.error("bge_reranker") is None and (self.loaded("bge_reranker") or (importlib.util.find_spec("sentence_transformers") is not None and cached(self.settings.reranker_model))),
            ...
            "foodsam": (self.settings.foodsam_model_dir/"zenfit_adapter.py").exists(),
            "foodseg103": (self.settings.foodseg_model_dir/"zenfit_adapter.py").exists(),
            "indian_food_classifier": self.settings.indian_food_model_path.exists(),
