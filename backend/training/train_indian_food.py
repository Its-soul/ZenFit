...
def main():
    p=argparse.ArgumentParser();p.add_argument("dataset",type=Path);p.add_argument("--models-dir",type=Path,default=Path("../data/models/indian_food"));p.add_argument("--config",type=Path,default=Path("training/configs/indian_food_v1.json"));p.add_argument("--version",required=True);p.add_argument("--dataset-version",required=True);p.add_argument("--epochs",type=int);p.add_argument("--batch-size",type=int);p.add_argument("--learning-rate",type=float);p.add_argument("--patience",type=int);p.add_argument("--smoke",action="store_true");args=p.parse_args();output=args.models_dir/args.version
