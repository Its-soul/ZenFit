...
def main():
    p=argparse.ArgumentParser();p.add_argument("version");p.add_argument("--models-dir",type=Path,default=Path("../data/models/indian_food"));p.add_argument("--dataset",type=Path,default=Path("/data/training/indian_food_v2"));p.add_argument("--raw",type=Path,default=Path("/data/raw/kaggle/food_image_classification/Food Classification dataset"));args=p.parse_args();root=args.models_dir/args.version
    import torch
