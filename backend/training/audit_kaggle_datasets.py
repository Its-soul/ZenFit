...
def audit(name,root):
    images=[p for p in root.rglob("*") if p.suffix.lower() in EXT];valid=[];corrupt=[];zero=[];digests={};exact=[];cross_class=[];near=defaultdict(list);classes=Counter()
    for path in images:
        ...
        try:
            ...
            if digest in digests:
                exact.append({"path":str(path),"same_as":digests[digest]})
                if path.parent.name!=Path(digests[digest]).parent.name:cross_class.append({"path":str(path),"class":path.parent.name,"same_as":digests[digest],"other_class":Path(digests[digest]).parent.name})
